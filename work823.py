...
# =============== НАСТРОЙКИ ===============
BOT_TOKEN = "8587482238:AAFYZLUZqJNv3-q7hdp88HvFHcEc7T-_8JU"  # ⚠️ ЗАМЕНИ НА СВОЙ ТОКЕН!
ADMIN_IDS = [7635015201, 8260588511]
...
class Database:
    ...
    def create_tables(self):
        ...
        self.cursor.execute('DROP TABLE IF EXISTS ban_logs')
...
async def check_ban(user_id) -> bool:
    ...
    if is_admin(user_id):  # ✅ ТЕПЕРЬ ПРОВЕРЯЕТ ПО СПИСКУ
        return False  # Админов не баним
...
<b>📢 КАНАЛ:</b> @blitz_ref_channel
<b>💬 ЧАТ:</b> @blitz_ref_chat
"""
    
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(text="👑 КАНАЛ", url="https://t.me/blitz_ref_channel"),
        InlineKeyboardButton(text="💬 ЧАТ", url="https://t.me/blitz_ref_chat")
    )
...
<i>👇 Выбери действие:</i>
    ...
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(text="📁 УПРАВЛЕНИЕ АККАУНТАМИ", callback_data="admin_load_category"),
...
async def admin_load_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Создаем клавиатуру с кнопками 1-50
    keyboard = InlineKeyboardBuilder()
    ...
    await callback.message.edit_text(
        ...
        "Выбери количество топов для загрузки:\n"
        "<i>(от 1 до 50)</i>\n\n"
...
<b>📊 Статус:</b> {ban_status}
        ...
        # Если это сообщение для рассылки
        elif current_state is None and len(parts) > 0:
